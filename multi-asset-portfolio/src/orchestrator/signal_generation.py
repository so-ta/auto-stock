...
2. Strategy evaluation on test data
...
class SignalGenerator:
    """
    Handles signal generation for the pipeline.
...
class StrategyEvaluator:
    ...
    def __init__(
        ...
        self._settings = settings
        self._audit_logger = audit_logger
        self._logger = logger.bind(component="strategy_evaluator")
    ...
    def evaluate_strategies(
        ...
        for symbol, signal_dict in signals.items():
            ...
            for signal_name, signal_info in signal_dict.items():
                try:
                    ...
                    # Get signal scores for test period
                    # Recompute signal on full data to get test period scores
                    from src.signals import SignalRegistry

                    signal_cls = SignalRegistry.get(signal_name)
                    signal_params = signal_info.get("params", {})
                    signal = signal_cls(**signal_params)
                    full_result = signal.compute(df)

                    # Extract test period scores
                    test_scores = full_result.scores.iloc[train_end_idx:]
                    ...
                    eval_result = StrategyEvaluationResult(
                        ...
                        walk_forward_summary={
                            ...
                            "optimized": signal_info.get("optimized", False),
                        },
                        score=score,
                        error_message=None,
                    ...
                    self._logger.warning(
                        f"Evaluation failed for {symbol}/{signal_name}: {e}"
                    )
                    total_errors += 1
        ...
        # Log audit info for each evaluation
        if self._audit_logger:
    ...
    def _compute_signal_returns(
        ...
        # Use index intersection for robust alignment
        common_idx = scores.index.intersection(returns.index)

        if len(common_idx) == 0:
            self._logger.debug(
                "No common index between scores and returns",
        ...
        self._logger.debug(
            "Aligned scores and returns",
            original_scores=len(scores),
            original_returns=len(returns),
            aligned=len(common_idx),
        )
    ...
    def _get_cost_per_trade(self) -> float:
        ...
        Returns:
            Cost per unit of turnover (in return terms)
        """
        # Get cost model from settings
    ...
    def _calculate_strategy_score(self, metrics: "StrategyMetrics") -> float:
        ...
        # Penalty coefficients (fixed)
        lambda_mdd = 0.5
        lambda_instability = 0.3

        # MDD penalty (normalized: 0-25% -> 0-1)
        mdd_penalty = lambda_mdd * (metrics.max_drawdown_pct / 25.0)

        # Instability penalty (based on period returns if available)
        ...
        return max(0.0, score)  # Clip negative scores to 0
