...
class TestSignalBase:
    ...
    def test_unknown_parameter_raises(self, sample_data):
        ...
        with pytest.raises(ValueError, match="Unknown parameters"):
            MomentumReturnSignal(unknown_param=123)
    ...
    def test_parameter_out_of_range_raises(self, sample_data):
        ...
        with pytest.raises(ValueError, match="out of range"):
            MomentumReturnSignal(lookback=1)  # min is typically > 1


class TestMomentumSignal:
    """Tests for MomentumReturnSignal."""
    ...
    def test_momentum_signal_parameters(self):
        """Test MomentumReturnSignal parameter specs."""
        from src.signals.momentum import MomentumReturnSignal

        specs = MomentumReturnSignal.parameter_specs()
...
class TestMeanReversionSignal:
    """Tests for BollingerReversionSignal (Bollinger Bands)."""
    ...
    def test_mean_reversion_signal(self, sample_data):
        ...
        from src.signals.mean_reversion import BollingerReversionSignal
...
class TestVolatilitySignal:
    """Tests for VolatilityBreakoutSignal."""
    ...
    def test_volatility_signal(self, sample_data):
        ...
        from src.signals.volatility import VolatilityBreakoutSignal

        signal = VolatilityBreakoutSignal(atr_period=14, ma_period=20)
        result = signal.compute(sample_data)
...
class TestSignalRegistry:
    ...
    def test_register_and_get_signal(self):
        ...
        # Use decorator syntax to register
        ...
        assert "mock_test_signal" in SignalRegistry.list_all()

        signal_cls = SignalRegistry.get("mock_test_signal")
        assert signal_cls == MockSignal

        # Clean up
        SignalRegistry.unregister("mock_test_signal")

    def test_get_unknown_signal_raises(self):
        """Test that getting unknown signal raises SignalRegistryError."""
